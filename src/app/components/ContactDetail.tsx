      ...
      <div className="flex flex-col md:flex-row gap-4 p-4 md:p-5 h-full items-stretch">
        ...
        <div className="w-full md:w-[38%] bg-white rounded-xl border border-gray-200 shadow-sm flex flex-col overflow-hidden">
          ...
          <div className="p-5 space-y-1">
            <h2 className="font-bold text-xl text-gray-800 leading-tight">{contact.name}</h2>
            ...
            {contact.industry && (
              <div className="inline-block text-xs bg-gray-100 text-gray-500 rounded-full px-2 py-0.5 mt-1">{contact.industry}</div>
            )}
            {contact.country && (
          ...
          <div className="px-5 pb-5 space-y-2 border-t border-gray-100 pt-4">
            {contact.email && (
              <button
                ...
                className="w-full text-left text-sm text-gray-600 flex items-center gap-2 hover:text-[#E8470A] hover:bg-orange-50 px-2 py-1.5 rounded-lg transition-colors"
              >
            ...
            {contact.phone && (
              <div className="text-sm text-gray-600 flex items-center gap-2 px-2 py-1.5">
            ...
            {contact.website && (
              <a
                ...
                className="text-sm text-gray-600 flex items-center gap-2 hover:text-[#E8470A] hover:bg-orange-50 px-2 py-1.5 rounded-lg transition-colors"
              >
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-4">
          <div className={`flex-1 flex flex-col border-[1.5px] rounded-xl overflow-hidden shadow-sm ${gradeBg}`}>
            <div className="p-4 flex justify-between items-center">
              <div>
                <div className={`font-bold text-lg ${gradeText}`}>
              ...
              <div className="text-right">
                <div className={`text-4xl font-black leading-none ${gradeText}`}>{classifyResult.score}</div>
                <div className="text-sm opacity-70">/ 100pt</div>
              </div>
            </div>
            ...
            <div className="flex-1 flex flex-col overflow-hidden bg-white border-t">
                    ...
                    return (
                      <div key={idx}>
                        <div className="flex justify-between items-center mb-1">
                          ...
                          <span className={`text-sm font-bold ${item.pts >= 0 ? 'text-emerald-600' : 'text-red-500'}`}>
                            {item.pts > 0 ? '+' : ''}{item.pts}pt
                          </span>
                        </div>
                        <div className="h-1.5 bg-gray-100 rounded-full mb-1.5">
                          <div
                            className={`h-1.5 rounded-full ${item.pts >= 0 ? 'bg-emerald-500' : 'bg-red-400'}`}
                            style={{ width: `${(Math.abs(item.pts) / maxPts) * 100}%` }}
                          />
                        </div>
                        {why && <p className="text-xs text-gray-400 leading-relaxed">{why}</p>}
                      </div>
                    );
                  })}
                </div>
              ...
              <div className="border-t pt-4">
                <div className="flex items-center gap-2 mb-2">
                  <div className="w-1 h-4 rounded-full bg-[#E8470A]" />
                  <span className="text-sm font-bold text-gray-700">추천 영업 목적</span>
                </div>
                <div
                ...
                >
                  <div className="font-bold text-lg" style={{ color: TYPE_COLORS[classifyResult.suggestedType]?.text || '#6B7280' }}>
                    ...
                    return (
                      <div key={item.key} className={`rounded-lg px-3 py-2 ${isCurrent ? 'bg-orange-50 ring-1 ring-[#E8470A]/30' : 'bg-gray-50'}`}>
                        <div className="flex justify-between items-center mb-1.5">
                          <span className={`text-xs font-medium truncate max-w-[70%] ${isCurrent ? 'text-[#E8470A]' : 'text-gray-600'}`}>
                            {item.emoji} {item.name}
                          </span>
                          <span className={`text-xs font-bold ml-2 flex-shrink-0 ${isCurrent ? 'text-[#E8470A]' : 'text-gray-500'}`}>
                            {item.score}pt
                          </span>
                        </div>
